...
/**
 * 대시보드(앱 홈, MAIN-00~09) — 3축의 현관. 다음 단계 히어로 + 최근 자산 그리드,
 * 첫 방문이면 셋업 가이드. 진행 중 잡은 우하단 플로팅 패널이 폴링으로 추적한다.
 */
...
export default async function DashboardPage() {
  ...
  const [requests, reports, assets, brandProfile] = await Promise.all([
    store.listRequests(),
    store.listReports(),
    store.listAssets(),
  ...
  const doneSteps = (brandProfile ? 1 : 0) + (publishedRequests.length > 0 ? 1 : 0) + (doneAssets.length > 0 ? 1 : 0);
            ...
            <div className="mx-auto max-w-[640px]">
              <div className="text-center">
                ...
                <h1 className="mt-4 text-[clamp(30px,3.4vw,38px)] leading-[1.25] font-extrabold tracking-[-0.03em] text-ink">
                  일본 진출, <b className="font-extrabold text-coral-strong">3단계</b>로 시작하세요
                </h1>
                <p className="mt-3 text-sm text-ink-mute">
                  <span className="tnum font-bold text-green-text">{doneSteps}/3 단계 완료</span>
                  {' · '}브랜드 컨텍스트가 3축 전체에서 재사용됩니다
                </p>
              </div>
                ...
                <ol className="m-0 list-none">
                  ...
                  <li className="flex items-start gap-3.5 py-4">
                    ...
                    <span className="min-w-0 flex-1">
                      <span className={`block text-[14.5px] font-bold text-ink ${brandProfile ? 'line-through decoration-ink/35' : ''}`}>
                        브랜드 프로필 등록
                      </span>
