...
## 현재 단계 — Phase 1 진행 중 (SP2 · 2026-09-08 ~ 09-19)

- **축이 둘이다 — 로드맵 `Phase {n}` 과 스프린트 `SP{n}`.** Phase 는 "무엇을 만드는 단계인가", SP 는 "이번 2주에 누가 무엇을"이다. **이름을 섞지 않는다**(2026-09-04 분리 — 그전에는 스프린트도 `Phase {n}` 이라 불러 헷갈렸다).
- **`Phase 0` (MVP 빌드, 2026-07-02 ~ 08-31) 종료.** 3축을 직접 개발하기로 확정(2026-07-04)한 뒤 배포·UT·리브랜딩·협업 기반 정비까지가 한 덩어리다.
- **`Phase 1` (2026-08-31 ~ 09-19) 진행 중** — "더 만들기"가 아니라 **만든 것을 믿고 쓸 수 있게**. 스프린트 둘로 굴린다:
  - **SP1** (8/31~9/04 · ✅ 종료) 상세 시간 예산 · 제품 선택 프리필 · 로그인 정리 — `M13~M15`
  - **SP2** (9/08~9/19 · 🔵 진행) 상세 폼 결과 중심 재구성 · 입력 자동 채움 · 제품 등록 개선 — `M16~M18`
- **Phase 정본 = `docs/04-roadmap.md`.** 구간·무엇을 만들었나·종료 시점 미해결이 전부 여기 있다. 마일스톤 상세는 `docs/09-dev-spec.md` §4~§4e(`M0~M12` = Phase 0 · `M13~M18` = Phase 1).
- **이번 스프린트 정본 = `docs/sprints/2026-09-sp2.md`** — 기간·담당·리스크·일자별. 다만 **일감의 상태 정본은 GitHub Issues** 다.
- **Phase 0 종료 판정:** 배포본에서 3축이 돌고 UT 를 마쳤다. 다만 **검증되지 않은 채로 종료했다** — 실사용자 수요·폐루프·지불 의사는 미검증이며 미결정 정본은 `docs/decisions/DECISIONS.md` "미결정" 절이다. **Phase 1 도 이 질문에는 답하지 않는다.**
