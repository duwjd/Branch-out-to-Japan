...
- 로고 정본은 `design/brand/logo/` — **규칙은 `design/brand/logo/README.md`를 읽고 따른다**(변형·여백·최소 크기·금지 사항).
  ...
  - 로고 색은 **UI 브랜드색과 통일**한다(2026-08-18 전환, 근거: `docs/decisions/2026-08-18-일출코랄-DS전환.md`). 잉크 `#182333` = `--color-ink`, 일출 코랄 `#FF6F61` = `--color-coral`. 구 스티비 coral `#ff6464`는 폐기 — 신규 산출물에 남아 있으면 교정한다.
    - 예외: **일출 그라디언트**(`#FF6F61`→`#FF9B70`)는 로고 심볼 전용이다. UI 면에는 단색 `#FF6F61`만 쓴다.
    - 코랄 위 소형 텍스트·링크는 `--color-coral-text`(`#C93F2E`)를 쓴다. 원색 `#FF6F61`은 대비 2.6:1이라 글자색으로 쓰지 않는다.
