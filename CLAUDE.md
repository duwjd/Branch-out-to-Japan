...
## 현재 단계
- **기획·페르소나 검증 단계 (지금)** — 합성 페르소나로 타겟·문구·가격 가설을 사전 검증. 랜딩 시안: `persona-simulation/landing/index.html` (검증용 정적 HTML, 비배포). 근거 문서: `docs/research/ 페르소나 검증형 MVP 실행안.md`
- Phase 1: 랜딩페이지 실배포 + 무료 "5분 진단" 신청으로 수요 검증 (검증 결과 반영 후 착수).
- Phase 2: 한 사이클을 경험하는 작동 서비스(범위 미정).
- 자세히: `docs/04-roadmap.md`
- ※ 기존 Next.js 앱 코드는 기획 단계 정리를 위해 제거됨 — git 이력에 보존되어 있어 언제든 복원 가능.
...
## 기술 스택
- 현재는 앱 코드 없음 (기획 단계). 검증용 랜딩 시안은 의존성 없는 단일 HTML.
- 개발 재개 시(Phase 1~2): Next.js (App Router) · TypeScript · Tailwind CSS · npm 재도입 예정. 저장소 루트 = 앱 루트.
