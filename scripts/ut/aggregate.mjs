...
function main() {
    ...
    for (const kind of ['thumbnail', 'detail']) {
      for (const img of m.generations?.[kind]?.images ?? []) {
        ...
        costRows.push([
          ...
          img.size,
          img.quality,
          img.usage?.input_tokens ?? '',
          d.image_tokens ?? '',
          d.text_tokens ?? '',
          img.usage?.output_tokens ?? '',
          img.usd ?? '',
          img.retry ?? 0,
        ]);
      }
  ...
  for (const m of manifests) {
    for (const t of ['T0', 'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7']) {
      ...
      taskRows.push([
        m.personaId,
        ARCHETYPE[m.personaId] ?? '',
        (LENS[m.personaId] ?? ['-']).join(';'),
        'A',
        t,
        outcome,
        '',
  ...
  for (const m of manifests) {
    ...
    personaRows.push([
      ...
      num(sc.det_dif),
      num(sc.det_tru),
      sc.best_asset_of3 ?? sc.best_asset ?? '',
      sc.wtp ?? '',
      num(sc.wtp_krw),
      num(sc.nps),
      sc.revisit ?? '',
      ...
      sc.slides_usable ?? '',
      sc.rep_new_insight ?? '',
      sc.changed_from_before ?? '',
      sc.rep_blocks_read ?? '',
      sc.rep_blocks_skipped ?? '',
      flags,
    ]);
    landingRows.push([
      m.personaId,
      a,
      l,
      num(sc.landing_appeal),
      num(sc.landing_interest),
      num(sc.landing_intent),
      sc.landing_scrolled_to_end ?? '',
      sc.landing_exit_section ?? '',
      sc.landing_hook_quote ?? '',
      sc.landing_friction_quote ?? '',
      flags,
    ]);
  ...
  log(
    `  scores-persona.csv — ${personaRows.length}행 · 랜딩평가.csv — ${landingRows.length}행${missing ? ` (응답 없음 ${missing}명)` : ''}`,
  );
  ...
  log(
    `  생성결과.csv — ${genRows.length}행 (성공 ${genRows.filter((r) => r[2] === 'done' || r[2] === 'published').length})`,
  );
