...
tokens:
  ...
  components:
    ...
    segmented: { type: tab, bg: "#f6f6f6", radius: 10, padding: "0 4px", font: "14px/400 Pretendard Variable", use: "Sort control track", active: "white segment 8px radius #414245 text" }
    reward-badge: { type: badge, fg: "#ffffff", radius: 2, font: "12px/600 Pretendard Variable", use: "합격보상금 overlay on thumbnail" }
    micro-tag: { type: badge, bg: "#747579", fg: "#ffffff", radius: 0, padding: "5px 7px", font: "10px/500 Pretendard Variable", use: "Footer legal pill" }
    search-input: { type: input, fg: "#202124", radius: 9999, font: "14px/400 Pretendard Variable", use: "Header search trigger" }
    avatar-pill: { type: avatar, bg: "#202124", radius: 9999, use: "Header user avatar 38x38" }
...
The brand color is a single, decisive **`#0066FF`** (PANTONE 2195 C) — a slightly cooler, more confident hue than Toss's `#3182f6`, intentionally positioned to feel like "growth blue" rather than "transaction blue." It appears only on interactive surfaces: the 회원가입/로그인 CTA, the apply button on job detail pages, link text, and selection states. Heading text sits at near-black `#202124`, body at `#414245`, and secondary metadata at `rgba(55,56,60,0.61)` — a translucent grey that lets backgrounds bleed through, signaling lower priority without committing to a hard hex value.
...
- White canvas (`#FFFFFF`) + warm-grey panel (`#F6F6F6`) — no dark mode on web (Montage system supports it)
...
- **Surface Subtle** (`#F6F6F6`): Segmented-control track, secondary panel background, filter dropdown menu surface.
- **Heading** (`#202124`): Primary text — H1/H2, position title, company name on JobCard.
- **Body** (`#414245`): Standard reading text, body paragraphs in job description.
...
- **Disabled** (`#747579`): Pre-disabled link state and inactive controls.
...
- **Divider** (`#EBEBEB`): Solid hairline for list separators inside content (job description sections).
...
- **Info** (`#FF6464`): Same as primary — info uses brand blue, not a separate teal.
...
- Background: transparent (inactive) → `#F6F6F6` (hover/active)
- Text: `#202124`
- Border: none
...
**Search Input (header)**
- Background: `#FFFFFF` inside translucent dark pill wrapper (`#202124`)
- Wrapper: `38×38px`, 9999px radius, `#202124` background
- Text: `#202124` placeholder
- Font: 14px / 400 / Pretendard Variable
...
- Shadow: none — relies on background contrast against container `#F6F6F6`
...
- Background: `#202124` (or `#FF6464` for brand emphasis)
- Text: `#FFFFFF`
- Border: none
- Radius: 2px
...
| Hover (Level 2) | Subtle bg shift to `#F6F6F6` | Filter chip hover, segmented-control segment |
...
**Shadow Philosophy.** Wanted prefers background-tint elevation over drop shadows. JobCards float on a white canvas without shadows — separation comes from the 20–24px grid gutter and the warm-grey `#F6F6F6` of the surrounding filter bar. Shadows only appear on floating overlays (dropdowns, modals), and they're single-layer pure black at low opacity — no colored tints, no multi-layer stacks. This matches Toss's restraint principle but is implemented through grid spacing rather than shadow tokens.

## 7. Do's and Don'ts

### Do
- Use `primary` (`#FF6464`) for default CTAs, primary buttons, and large emphasis; use `primary-strong` (`#D93636`) for hover / pressed / active and for small interactive text (text links) where white/small text needs AA. Reserve `error` (`#F0483C`) for validation / destructive states only — never a CTA
...
- Primary / Default CTA: `#FF6464` (Coral) — default CTA, primary button, large emphasis
- Primary Strong (`#D93636`): hover / pressed / active · text-link & small-text contrast (NOT default CTA bg) · distinct from error `#F0483C`
- Heading text: `#202124`
- Body text: `#414245`
...
2. The default CTA / primary button is `#FF6464`; `#D93636` (`primary-strong`) is only for hover / pressed / active and small-text contrast — never the default CTA background, and never confused with `error` `#F0483C`
