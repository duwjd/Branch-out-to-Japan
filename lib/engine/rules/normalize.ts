...
import type { NormalizedContent, Sentence, TierInput } from '../types';

const HARD_GATE_CHARS = 50;
const SOFT_LINE_CHARS = 200;
...
async function fetchUrlText(url: string): Promise<string> {
  ...
  try {
    ...
    throw new SourceContentError(
      `URL을 가져오지 못했습니다(${String((err as Error)?.message ?? err)}). 상세페이지 텍스트를 직접 붙여넣어 주세요 — 이미지 위주 페이지는 붙여넣기가 가장 정확합니다.`,
    );
  }
  const text = stripHtml(html);
  if (text.length < HARD_GATE_CHARS) {
    throw new SourceContentError(
      'URL에서 추출된 텍스트가 너무 적습니다(이미지 위주 상세로 보입니다). 상세페이지 문구를 직접 붙여넣어 주세요.',
    );
  }
  return text;
}
...
/** 티어 입력의 source를 정규화 산출물로 변환한다 */
export async function normalizeContent(input: TierInput): Promise<NormalizedContent> {
  const raw =
