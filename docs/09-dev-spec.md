...
>
> **Phase 경계.** M12-A 만 Phase 0 안에서 끝났다(`c1f3d30`). **M12-B 는 Phase 1 SP1 에서 M14 로 흡수**됐고(제품 선택 프리필이 폐루프 배선과 같은 코드 자리였다), **M12-C 의 UT-26·UT-31 은 SP2 M16 이 집는다**. 나머지(M12-C 잔여 · M12-D · M12-E)는 여전히 미착수다 — §4e 참조.
...
## 4e. Phase 1 마일스톤 (M13~M18 · 스프린트 둘로 굴린다)

> **[[04-roadmap]] §Phase 1 의 실행 단위.** 기간·담당·리스크·일자별 기록은 스프린트 문서가, **일감의 상태는 GitHub Issues 가** 정본이다 — 여기는 "무엇을 왜 만드는가"만 적는다([[sprints/운영-가이드]] §0).
>
> 번호는 Phase 0 의 `M0~M12` 를 이어 간다. 마일스톤은 GitHub 라벨(`M13`…`M18`)로 표현하고 **GitHub Milestones 기능은 쓰지 않는다**.

### SP1 (2026-08-31 ~ 09-04 · ✅ 종료) — [[sprints/2026-09-sp1]]

**M13 · 상세페이지 시간 예산** — 실패가 나면 어느 단계가 얼마를 먹었는지 말할 수 있게 한다.
- [x] **M13-1** 단계별 소요 계기 — `detail_input` jsonb 안에 `timings`(신규 컬럼 없이 · `theme`·`humanizeIssues` 선례). **실패 경로에도 남긴다**
- [x] **M13-2** 콜⑦⑧⑨ 개별 타임아웃 — M12-A 선례 이식. **SDK 기본(10분)이 함수 상한(300초)보다 길다**는 것이 리포트를 20/20 죽였던 실제 원인이다
- [ ] **M13-3** 예산 상수 재조정 — **SP2 로 이월.** 실측이 쌓여야 한다(막힌 게 아니라 하지 않기로 한 것)
- **DoD 미달:** 콜⑦ 타임아웃은 여전히 잡 전체를 `failed` 로 끝낸다. 「예산 초과가 강등으로 끝난다」는 M13-3 에 걸려 있다

**M14 · 제품 선택 프리필** — 입력을 줄이면서 UT P0 셋을 같은 코드 자리에서 함께 닫는다(M12-B 흡수).
- [x] **M14-1** `generated_assets`·`diagnosis_requests` 에 `product_id` — 스토어 3곳(`store`·`fileStore`·`supabaseStore`)을 함께
- [x] **M14-2** 상세 폼 맨 앞 제품 선택 (UT-58) · **M14-3** 지난 입력 프리필 · **M14-4** UT-25 한국어 노출 · UT-33 타입 오판
- [x] **M14-5** 2회차 필수 입력을 제품 선택 하나로 — 계획에 없던 추가. 티켓을 다 닫고도 **완료 판정을 못 넘겨** 발견했다
- **DoD 달성:** 지난 생성이 있는 제품을 고르면 그 즉시 생성이 눌린다

**M15 · 로그인** — 인증 없이 세션을 받는 경로를 닫는다.
- [x] **M15-1·2** 목 세션 라우트 차단 + 레거시 원시 쿠키 경로 제거 + 이메일 단독 UI → [[decisions/2026-08-31-목-소셜로그인-차단]]
- [x] **M15-3a** `AUTH_MAIL_MODE` 임시 플래그 제거 · 인증 링크 노출을 **정식 경로로 승격** → [[decisions/2026-09-02-실메일-보류-devlink-정식화]]
- [ ] **M15-3b** 실발송 E2E — **범위 밖.** 실 발송을 하지 않기로 해 검증 대상이 없다
- **감수:** 이메일 소유 검증이 성립하지 않는다. **폐쇄 UT 전제에서만 유효하며 공개 모집 전 해제 필수**

### SP2 (2026-09-08 ~ 09-19 · 🔵 진행) — [[sprints/2026-09-sp2]]

> SP1 이 상세페이지를 **믿고 쓸 수 있게** 만들었으니 SP2 는 **쓰기 쉽게** 만든다.

**M16 · 상세 폼 결과 중심 재구성** — 입력이 아니라 **결과물**이 화면의 주어가 된다.
- 진단: `checkRequirement()` 가 블록별로 `{reason, fixHint}` 를 이미 돌려주는데(`lib/studio/detail/blockPack.ts`) **확인 단계에서만** 렌더된다. 사용자는 37칸을 다 채운 **뒤에야** 무엇이 필요했는지 안다
- [ ] **M16-1** `requires`→폼 필드 매핑(`fields: string[]`) + **LLM 없는** `POST /api/studio/detail/outline`. 기존 `/plan` 은 콜⑧ 번역을 태워(최악 22초·과금) 실시간 갱신에 못 쓴다
- [ ] **M16-2** 필수 최소 폼 — 제품·KR 원본·템플릿 셋 · [ ] **M16-3** 블록 보드(핵심 화면) · [ ] **M16-4** UT-26·UT-31 · [ ] **M16-5** DS 정합·접근성
- **DoD:** 확인 단계에서 처음 알게 되는 것이 없다 · 빠진 블록을 "숙제"로 읽는 구간이 없다
- **근거:** 이 화면이 UT **관찰 지점 2위(7/20)** — *"이거 다 채우려면 시간이 더 드는데 싶어 잠깐 멈칫했다"*(P15)

**M17 · KR 원본에서 스펙 자동 추출** — 이미 올리는 자료에서 뽑는다.
- [ ] **M17-1** KR 상세 원본 비전 → 스펙 7칸(용량·제조사·원산지·전성분·성분표·사용법·주의사항). `runStructuredCall` 재사용 · **`timeoutMs` 필수**(M13-2 선례)
- [ ] **M17-2** 자동 채움 + **출처 배지** + 규정 가드 · [ ] **M17-3** 실패·부분 성공 경로
- **규정 가드:** 이 서비스에는 약기법 게이트가 있다(`assembleBlockSlots`). **전성분·효능·시험 결과는 사용자 확인 전까지 해당 블록을 붙이지 않는다**

**M18 · 제품 등록 웹 검색 보조** — 원본에 **없는 것**을 채운다.
- [ ] **M18-1** 제품컷 비전 + 서버 툴 `web_search_20260209`. ⚠ **서버 툴 오류는 예외를 던지지 않는다** — 성공이면 `content` 가 배열, 실패면 객체다
- [ ] **M18-2** 후보 선택 UI + **0건 폴백**(2~30인 브랜드의 신제품은 웹에 없다 — 0건이 정상 경로다)
- **왜 보조인가:** `products` 는 필드가 4개뿐이라 웹 검색 단독으로는 절약폭이 작다. 진짜 피로는 상세 폼 37칸이고 그중 스펙 7칸은 M17 이 집는다

**품질 가드(M16~M18 공통)** — 입력을 줄이는 변경이 산출물을 얇게 만들면 실패다. **같은 제품·같은 자료로 재생성했을 때 붙는 블록 수가 줄지 않아야 한다.** 회귀 케이스를 `blockPack.test.ts` 에 고정한다.
