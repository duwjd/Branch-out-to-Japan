...
| `DEMO_ACCOUNT_EMAIL` · `DEMO_ACCOUNT_PASSWORD` | 선택 — 로그인 화면 "데모 계정 체험하기" 버튼이 쓸 계정(시연용, 2026-09-19). **해당 DB 에 가입·인증된 계정**이어야 한다 | 선택(같은 값 가능) | 둘 중 하나라도 비면 버튼을 숨기고 `/api/auth/demo` 는 404 |
