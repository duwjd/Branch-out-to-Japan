...
- 파일명은 `YYYY-MM-sp{n}.md`. 스프린트 하나에 문서 하나.
- **스프린트는 `SP{n}`, 로드맵은 `Phase {n}`** — 다른 축이라 이름을 나눈다(2026-09-04 개명. 그전에는 스프린트도 `Phase {n}` 이라 불러 [[../04-roadmap]] 의 Phase 와 헷갈렸다).
- 끝난 스프린트도 **지우지 않는다.** 회고가 다음 스프린트 범위의 입력값이다.
...
| 스프린트 | 기간 | 목표 | 상태 |
|---|---|---|---|
| [SP1](2026-09-sp1.md) | 2026-08-31 ~ **09-04 조기 종료** | 상세페이지 안정화 · 입력 피로 해소 · 로그인 개선 | ✅ 종료 (목표 2·3 달성 · 목표 1 절반) |
