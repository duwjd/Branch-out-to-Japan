...
이 셋업에는 **배포 파이프라인에 붙은 자동 마이그레이션이 없다.** 코드가 새 테이블·컬럼을 기대하도록 바뀌면 사람이 실행해야 한다 — 다만 **실행 자체는 명령 한 줄로 된다**(`npm run db:push`, `.env` 에 `SUPABASE_DB_URL` 필요 · [setup-supabase.md §5](setup-supabase.md)). 대시보드 SQL Editor 경로도 그대로 유효하다.
